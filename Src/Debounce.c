...
static bool gIsKeyPressedDebounced = false ; ///> This is the global variable that maintains the debounced key state 
 ...
 * 
 * The pressMask determines the press time of the button. This is to be a multiple of button polling interval  
 * A press mask 0xF000 (0b 1111 0000 0000 0000)would mean a Pres time of 4 * (button read time) 
 * 
 ...
 * 
 * The debounce release mask determines how much debouncing is added, This is to be a multiple of button polling interval  
 * A debounce mask of 0xF000 (0b 1111 0000 0000 0000) would mean a debouncing period of 4 * (button read time) 
 * 
 * @param pMe : handler of type pointer to @ref sDebounceHelper_t
 ...
 * 
 * @param pMe : handler of type pointer to @ref sDebounceHelper_t
 * @param buttonReadTimeInMs : Frequency with which button is being read from the hardware 
 ...
 */
eDebounceStatus_t debounce_Init(sDebounceHelper_t* const pMe, uint8_t buttonReadTimeInMs, uint8_t pressTimeInMs, uint8_t releaseTimeInMs, pf_IsKeyPressed pfIsKeyPressed )
{
    assert(NULL != pMe);
    assert(NULL != pfIsKeyPressed);
    assert((buttonReadTimeInMs < pressTimeInMs) && (buttonReadTimeInMs < releaseTimeInMs) );
 ...
 * 
 * This function needs to be called periodically at the rate set in @ref debounce_Init routine
 * The result of if the button was pressed or not is updated in member r_IsKeyPressed of @ref sDebounceHelper_t
 * The result of it the key was changed or not is updated in member r_IsKeyChanged of @ref sDebounceHelper_t
 * 
 * @param pMe : handler of type pointer to @ref sDebounceHelper_t
 * @return eDebounceStatus_t  return status 
 */
eDebounceStatus_t debounce_pressRelease(sDebounceHelper_t* const pMe)
{
    ...
    ///> if the current key press state and global key press state are same, update count  
    if(isRawKeyPressed == gIsKeyPressedDebounced)
    {
        ///> setup the timer to monitor next change 
        count = (true == gIsKeyPressedDebounced)? releaseTimeCount : pressTimeCount ;
    }
    else
    {
...
eDebounceStatus_t debounce_bitTrain(sDebounceHelper_t* const pMe)
{
    ...
    bool prevIsKeyPressed = pMe->r_IsKeyPressed; ///> Setting up previous state by copying state before update 
    ...
    ///> Pin readings converted to bit train here. @ref debounceIgnoreMask creates a ignore condition on the bounces 
    state = (state<<1) | pMe->w_IsKeyPressedFunction() |  debounceIgnoreMask;
    pMe->r_IsKeyPressed = (state == pressedMask)? true : false ; 
    pMe->r_IsKeyChanged = (prevIsKeyPressed == pMe->r_IsKeyPressed)? true : false ;
